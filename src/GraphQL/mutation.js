...
export const UpdateQuestion = gql`
  mutation MyMutation($id: Int!, $title: String!, $question: String!, $code: String!, $objects: [tag_insert_input!]!) {
    update_question_by_pk(pk_columns: {id: $id}, _set: {title: $title, question: $question, code: $code}) {
      id
    }
    delete_tag(where: {question_id: {_eq: $id}}) {
      affected_rows
...
export const DeleteUserBlog = gql`
  mutation MyMutation($id: Int!) {
    ...
    delete_blog_tags(where: {blog_id: {_eq: $id}}) {
      affected_rows
    }
